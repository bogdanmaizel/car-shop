...
void showBrand(int noOfBrands, char brands[][10]);
    ...
    while(!contractSigned){
        switch (state) {
            ...
            case 2: {
                ...
                // consume new line
                getchar();
                getChoiceIndex(noOfBrands,state);
                break;
            }
            case 3: {
...
int getChoiceIndex(int noOfChoices, int *state) {
    int choiceIndex; char choice=getchar();
    getchar();
    if(choiceIndex == 'a'+noOfChoices) {
        (*state)--;
    }
    else {(*state)++;
        choiceIndex = choiceIndex - 'a';}
}
